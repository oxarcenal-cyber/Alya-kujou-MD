...
export default class command extends BaseCommand {
    ...
    private handleSearch = async (
        ...
        if (!context) return void M.reply(t('nsfw_nh_no_query', lang))
        return await nhentai
            ...
            .catch(async () => void (await M.reply(t('nsfw_nh_not_found', lang, { query: context.trim() }))))
    }

    private handleDownload = async (M: Message, { id, output }: TOption, nhentai: NHentai): Promise<void> => {
        const lang = await this.getLang(M)
        if (id === '') return void M.reply(t('nsfw_nh_no_id', lang))
        if (output === '') output = 'pdf'
        const valid = await nhentai.validate(id)
        if (!valid) return void M.reply(t('nsfw_nh_invalid_id', lang, { id }))
        await delay(1500)
        return await nhentai
            .getDoujin(id)
            .then(async (res) => {
            ...
            .catch(async () => void (await M.reply(t('nsfw_nh_try_again', lang))))
    }
