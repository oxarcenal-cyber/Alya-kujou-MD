...
interface WheelSlot {
    label: string
    emoji: string
    multiplier: number   // 0 = lose, >1 = win
    weight: number
}
...
function buildWheelFrame(spinning: WheelSlot[], highlight: number, amount: number, phase: string): string {
    const display = spinning.map((sl, i) =>
        i === highlight ? `❱${sl.emoji} ${sl.label}❰` : `  ${sl.emoji} ${sl.label}  `
    )
...
export default class extends BaseCommand {
    public override execute = async (M: Message, { args }: IArgs): Promise<void> => {
        ...
        const sent = await this.client.sendMessage(
            M.from,
            { text: buildWheelFrame(
                Array.from({ length: visibleCount }, (_, i) => SLOTS[(startIdx + i) % SLOTS.length]),
                2, amount, `🌀 *Spinning...*`
            )},
        ...
        await sleep(600)
        await edit(
            buildWheelFrame(centeredSlots, 2, amount, `\n🎯 *RESULT: ${result.emoji} ${result.label}*\n\n${resultLine}`)
        )
        ...
        if (result.multiplier === 0) {
            ...
            if (gifBuf) this.client.utils.gifToMp4(gifBuf).then(mp4 => this.client.sendMessage(M.from, { video: mp4, gifPlayback: true, mimetype: 'video/mp4', caption: `💔 *LOSE! -${amount.toLocaleString()} gold!*` })).catch(() => {})
        }
    }
}
