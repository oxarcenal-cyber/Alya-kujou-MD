...
interface Stock {
    ...
    emoji: string
    basePrice: number
    volatility: number   // 0.05 = 5% max swing per tick
}

interface StockPrice {
    price: number
    change: number       // % change from last tick
    history: number[]    // last 5 prices
}
...
const portfolios = new Map<string, Map<string, UserHolding>>()
...
function getOrInitPortfolio(jid: string): Map<string, UserHolding> {
    if (!portfolios.has(jid)) portfolios.set(jid, new Map())
    return portfolios.get(jid)!
}

function trend(history: number[]): string {
...
export default class extends BaseCommand {
    public override execute = async (M: Message, { context }: IArgs): Promise<void> => {
        ...
        if (sub === 'info') {
            ...
            if (!stock) return void M.reply(`❌ Stock *${sym}* nahi mila!\n📢 \`${prefix}stock market\` se list dekho`)
        ...
        if (sub === 'buy') {
            ...
            const stock  = STOCKS.find(s => s.symbol === sym)
            if (!stock)  return void M.reply(`❌ Stock *${sym}* nahi mila!\n📢 \`${prefix}stock market\``)
            if (!qty || qty < 1) return void M.reply(`❌ Kitne shares? Min *1*\n📢 Example: \`${prefix}stock buy ${sym} 5\``)
            if (qty > 1000) return void M.reply(`❌ Max *1000 shares* ek baar mein`)
            ...
            if (wallet < total)
                return void M.reply(
                    `❌ Wallet mein *${wallet.toLocaleString()} gold* hai\n` +
        ...
        if (sub === 'sell') {
            ...
            if (!qty || qty < 1) return void M.reply(`❌ Kitne shares becho? Ya \`all\`\n📢 Example: \`${prefix}stock sell ${sym} 5\``)
            if (qty > holding.shares) return void M.reply(`❌ Tumhare paas sirf *${holding.shares} shares* hain!`)
            ...
            holding.shares -= qty
            if (holding.shares === 0) port.delete(sym)
            else port.set(sym, holding)

            return void M.reply(
                `✅ *${qty} ${sym} shares bech diye!*\n\n` +
                `${stock.emoji} *${stock.name}*\n` +
