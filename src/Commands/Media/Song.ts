...
export default class extends BaseCommand {
    public override execute = async (M: Message, { context }: IArgs): Promise<void> => {
        ...
        // Search YouTube
        const searchData = await this.client.utils.fetch<YTSearchResponse>(
        ...
        const mp3Data = await this.client.utils.fetch<YTMp3Response>(
            `https://apis.davidcyriltech.my.id/download/ytmp3?url=${encodeURIComponent(top.url)}`
        ).catch(() => null)
