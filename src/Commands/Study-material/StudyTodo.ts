...
// ── In-memory study todo storage: per user JID ────────────────────────────────
interface TodoItem { task: string; done: boolean; addedAt: string }
const userTodos = new Map<string, TodoItem[]>()

const getTodos = (jid: string): TodoItem[] => {
    if (!userTodos.has(jid)) userTodos.set(jid, [])
...
export default class StudyTodoCommand extends BaseCommand {
    override execute = async (M: Message, { args }: IArgs): Promise<void> => {
        ...
        if (sub === 'add') {
            ...
            if (todos.length >= 30)
                return void M.reply(
                    `❌ You've reached the *30 task limit!*\n\n` +
        ...
        if (sub === 'list') {
            if (!todos.length)
                return void await this.client.sendMessage(M.from, {
                    text:
                        `📌 *Your Study To-Do List is Empty!*\n\n` +
