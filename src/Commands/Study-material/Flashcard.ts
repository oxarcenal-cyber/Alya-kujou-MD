...
// ── In-memory flashcard storage: per user JID ────────────────────────────────
interface Card { term: string; definition: string }
const userCards = new Map<string, Card[]>()

const getCards = (jid: string): Card[] => {
    if (!userCards.has(jid)) userCards.set(jid, [])
...
export default class FlashcardCommand extends BaseCommand {
    override execute = async (M: Message, { args, context }: IArgs): Promise<void> => {
