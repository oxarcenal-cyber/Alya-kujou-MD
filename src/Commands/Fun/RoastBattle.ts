...
const pending = new Map<string, RoastSession>()
...
function getFallbackRoast(arr: string[]): string {
...
async function getAiRoast(attackerName: string, defenderName: string, fallbackArr: string[]): Promise<string> {
    const ai = await askGroq(
        `Generate ONE savage, witty, single-line roast in Hinglish (Hindi+English mix). ` +
...
export default class extends BaseCommand {
    public override execute = async (M: Message, { context }: IArgs): Promise<void> => {
        ...
        if (input === 'cancel') {
            const session = pending.get(group)
            if (!session) return void M.reply('❌ Koi pending battle nahi!')
