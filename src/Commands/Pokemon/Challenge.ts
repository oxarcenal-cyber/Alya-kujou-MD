    ...
    override execute = async (M: Message, { context }: IArgs): Promise<void> => {
        ...
        if (context?.trim().toLowerCase() === 'info' || context?.trim().toLowerCase() === 'help') {
            return void await this.client.sendMessage(M.from, {
                text:
                    `📖 *How the Gym Trainer Challenge works*\n\n` +
        ...
        if (!gym || gym.expiresAt < Date.now()) {
            return void await this.client.sendMessage(M.from, {
                text:
                    `🌫️ *No Gym Leader here right now.*\n\n` +
        ...
        if (party.length < 1) {
            return void await this.client.sendMessage(M.from, {
                text:
                    `❌ *You need at least one Pokémon in your party to battle!*\n\n` +
        ...
        if (!won) {
            await replyWithPokemonImage(
                M, 'lose',
                `${gym.type.emoji} Your *${this.client.utils.capitalize(strongest.name)}* (Lv. ${strongest.level}) lost to *${this.client.utils.capitalize(gym.name)}* (Lv. ${gym.level})! 💥\n\nTrain harder and try again!`
            )
            ...
            if (loseGifBuf) this.client.utils.gifToMp4(loseGifBuf).then(mp4 => this.client.sendMessage(M.from, { video: mp4, gifPlayback: true, mimetype: 'video/mp4' })).catch(() => {})
        ...
        await replyWithPokemonImage(M, 'win',
            `🎉 *VICTORY!* 🎉\n\n` +
        ...
        if (winImg) this.client.sendMessage(M.from, { image: winImg, caption: `🏆 *${M.sender.username}* defeated the Gym Leader! 🏆` }).catch(() => {})

        // Claim options as Open Menu button
        return void await this.client.sendMessage(M.from, {
            text:
                `💰 *${p}claim currency* — ${currency} coins\n` +
