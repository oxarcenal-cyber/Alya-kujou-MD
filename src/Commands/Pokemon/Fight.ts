...
import { getRaidProgressBar } from '../../lib/TeamRocket'
    ...
    public override execute = async (M: Message, {}: IArgs): Promise<void> => {
        ...
        if (!raid) {
            return void await this.client.sendMessage(M.from, {
                text:
                    `😌 *No active Team Rocket raid here!*\n\n` +
        ...
        if (raid.fighters.has(M.sender.jid))
            return void await this.client.sendMessage(M.from, {
                text:
                    `⚔️ *You already fought in this raid!*\n\n` +
        ...
        const progress  = getRaidProgressBar(raid.totalDamage, raid.requiredDamage)

        // ── VICTORY ───────────────────────────────────────────────────────────────
        if (raid.totalDamage >= raid.requiredDamage) {
            if (raid.timer) clearTimeout(raid.timer)
            this.handler.rocketRaids.delete(M.from)

            // Return Pokémon to victim
            try {
                const victim = await this.client.DB.getUser(raid.victimJid)
                victim.party.length < 6 ? victim.party.push(raid.stolenPokemon as any) : victim.pc.push(raid.stolenPokemon as any)
                await this.client.DB.user.updateOne(
                    { jid: raid.victimJid },
                    { $set: { party: victim.party, pc: victim.pc } }
                )
            ...
            await Promise.allSettled(
                fighterJids.map((jid) => this.client.DB.setCrystal(jid, 50).catch(() => {}))
            )
            ...
            const rocketDefeatImg = this.client.assets.get('rocket-raid-defeat') as Buffer | undefined
            rocketDefeatImg
        ...
        return void this.client.sendMessage(M.from, {
            text:
                `⚔️ *@${M.sender.jid.split('@')[0]}* attacked and dealt *${damage} damage!*\n\n` +
                `${progress}\n` +
