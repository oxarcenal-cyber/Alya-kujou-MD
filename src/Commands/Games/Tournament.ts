...
const tournaments = new Map<string, TournamentSession>()
...
async function sendFightGif(M: Message, client: InstanceType<any>, key: 'fight-normal' | 'fight-legendary' | 'fight-winner' | 'fight-create', caption: string): Promise<void> {
    const buffer = client.assets.get(key) as Buffer | undefined
    if (!buffer) return
...
async function simulateMatch(
    ...
    await sendFightGif(
        M, client,
        isLegendary ? 'fight-legendary' : 'fight-normal',
        isLegendary
    ...
    if (!sent?.key) return Math.random() > 0.5 ? p1 : p2

    const edit = (t: string) => client.sendMessage(M.from, { text: t, edit: sent.key } as any)

    while (p1.hp > 0 && p2.hp > 0) {
        const a1 = calcDmg(); p2.hp = Math.max(0, p2.hp - a1.dmg)
        ...
        await sleep(900); await edit(header())
        if (p2.hp <= 0) break

        const a2 = calcDmg(); p1.hp = Math.max(0, p1.hp - a2.dmg)
    ...
    if (loseGifBuf) client.utils.gifToMp4(loseGifBuf).then(mp4 => client.sendMessage(M.from, { video: mp4, gifPlayback: true, mimetype: 'video/mp4', caption: `💀 *${loser.emoji} ${loser.name}* eliminated!` })).catch(() => {})
    ...
    public override execute = async (M: Message, { context }: IArgs): Promise<void> => {
        ...
        if (input.startsWith('create')) {
            ...
            setTimeout(() => {
                ...
                if (t && !t.started) tournaments.delete(group)
            }, 120_000)

            await sendFightGif(
                M, this.client, 'fight-create',
                `🏆 *TOURNAMENT CREATED!*\n\n` +
                `👑 Host: *${hostName}*\n` +
        ...
        if (input === 'join') {
            const t = tournaments.get(group)
            if (!t) return void M.reply(`❌ Koi tournament nahi!\n📢 Banao: \`${prefix}tour create <amount>\``)
            if (t.started) return void M.reply('❌ Tournament already shuru ho gaya!')
            if (t.players.length >= 8) return void M.reply('❌ Max 8 players!')
            if (t.players.find(p => p.jid === fromJid)) return void M.reply('❌ Tum pehle se joined ho!')

            const { wallet } = await this.client.DB.getUser(fromJid)
            if (wallet < t.bet) return void M.reply(`❌ Tumhare paas *${t.bet.toLocaleString()} gold* nahi!`)
            ...
            return void await this.client.sendMessage(M.from, {
                text:
                    `✅ *${M.sender.username || 'Fighter'}* joined! ${emoji}\n\n` +
        ...
        if (input === 'cancel') {
            const t = tournaments.get(group)
            if (!t) return void M.reply('❌ Koi tournament nahi!')
            if (t.hostJid !== fromJid && !M.sender.isAdmin) return void M.reply('❌ Sirf host ya admin cancel kar sakta hai!')
        ...
        if (input === 'start') {
            const t = tournaments.get(group)
            if (!t) return void M.reply(`❌ Koi tournament nahi!\n📢 Banao: \`${prefix}tour create <amount>\``)
            if (t.started) return void M.reply('❌ Already shuru ho gaya!')
            if (t.hostJid !== fromJid && !M.sender.isAdmin) return void M.reply('❌ Sirf host ya admin start kar sakta hai!')
            if (t.players.length < 2) return void M.reply(`❌ Min *2 players* chahiye!`)

            t.started = true
            const prize = t.bet * t.players.length

            // Deduct bets
            for (const p of t.players) await this.client.DB.setCrystal(p.jid, -t.bet)
            ...
            while (remaining.length > 1) {
                ...
                await M.reply(
                    `⚔️ *ROUND ${roundNum} — ${matches.length} MATCHES*\n\n` +
                ...
                for (let i = 0; i < matches.length; i++) {
                    ...
                    m.done   = true
                    nextRound.push(w)
                    if (i < matches.length - 1) await sleep(1500)
                }
            ...
            await sendFightGif(
                M, this.client, 'fight-winner',
                `👑🎉 *${champion.emoji} ${champion.name}* is the CHAMPION! 🎉👑`
            )
            await sleep(500)
            ...
            const winImg = this.client.assets.get(Math.random() < 0.5 ? 'winner-1' : 'winner-2') as Buffer | undefined
            if (winImg) {
