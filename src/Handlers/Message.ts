...
import { ICharacter as WaifuResponse } from '@shineiichijo/marika'
import { IQuiz } from 'anime-quiz'
import { Message, Client, BaseCommand } from '../Structures'
import { getStats, askRias, getRandomGreeting, getRandomReactEmoji, getPersona, getPersonaName, t, getCuteSticker, getBeastReply, beastDelay, getRandomIntroVideo } from '../lib'
import type { ReactMode, Language } from '../lib'
import { ICommand, IArgs, IPokemonAPIResponse } from '../Types'
import { Pokemon } from '../Database'
import type { TGroupModel } from '../Database/Models/Group'
import { getRarity, RARITY_META } from '../lib/PokemonRarity'
import { pickGymLeader, IGymChallenge, IGymReward } from '../lib/GymLeader'
import { getRandomCard, getCardPrice, fetchSpawnCard, TIER_EMOJI, TIER_NAME, isGif } from '../lib/CardData'
import { pendingGives } from '../Commands/Economy/Give'
import { roxyAwaitingUsers, handleRoxyReply, askRoxy, detectLang, getProfile, updateProfile, ROXY_INTRO, ROXY_NO_AI } from '../lib/RoxyBrain'
import { SMASHBOOM_LINES } from '../lib/SmashBoomLines'
import { ITeamRocketRaid, IRocketMember, getRocketMember, getRaidProgressBar, ROCKET_MEMBERS } from '../lib/TeamRocket'
import { PvpBattle, calcDamage, hpBar } from '../lib/PvpBattleState'
...
export class MessageHandler {
    ...
    public smashboom: string[] = []

    public wildPaused = false

    public cardsPaused = false

    public gymChallenge = new Map<string, IGymChallenge>()

    public gymReward = new Map<string, IGymReward>()

    public rocketRaids = new Map<string, ITeamRocketRaid>()

    // ─── Analytics batch buffer — flush to DB every 30s instead of per-message ──
    private _analyticsGroupTotal = new Map<string, number>()       // groupJid → total msgs
    private _analyticsMemberCount = new Map<string, number>()      // `groupJid|senderJid` → count

    private _flushAnalytics = async (): Promise<void> => {
        if (this._analyticsGroupTotal.size === 0 && this._analyticsMemberCount.size === 0) return
        ...
        const memberBulkInc: any[] = []
        const memberBulkPush: any[] = []
        ...
        if (memberBulkInc.length > 0) {
            const results: any = await this.client.DB.group
            ...
            if (results && results.modifiedCount < memberBulkPush.length) {
                const pushBulk: any[] = memberBulkPush.map(({ jid, senderJid, count }) => ({
                    updateOne: {
    ...
    public startAnalyticsFlush = (): void => {
        setInterval(() => { this._flushAnalytics().catch(() => {}) }, 30_000)
    }

    // ─── Prefix-only menu: same format as List.ts command (buttons with sections) ─
    private _buildPrefixMenuMsg = (nsfwEnabled = false): object => {
    ...
    private buildMenuList = (nsfwEnabled = false): object => {
        const categoryIcons: Record<string, string> = {
            general:    '🌐',
            games:      '🎮',
            economy:    '💰',
    ...
    public greetGroups: string[] = []

    /** @deprecated — use prefetched groupData from handleMessage instead */
    private getGroupLang = async (M: Message): Promise<Language> => {
    ...
    private spawnGymChallenge = (): void => {
        schedule('0 */2 * * *', () => {
            ...
            for (let i = 0; i < this.wild.length; i++) {
                ...
                setTimeout(async () => {
                    ...
                    if (this.gymChallenge.has(groupJid)) return void null
                    const { type, pokemonId, level } = pickGymLeader()
    ...
    private spawnPokemon = async (): Promise<void> => {
        schedule('*/7 * * * *', async () => {
            if (this.wildPaused || this.wild.length < 1) return void null
            for (let i = 0; i < this.wild.length; i++) {
                setTimeout(async () => {
                    try {
                        ...
                        this.client.log(
                            `[Wild spawn] skipped while WhatsApp is unavailable: ${(error as Error).message}`,
                            true
                        )
                        this.pokemonResponse.delete(this.wild[i])
                        this.pokemonSpawnInfo.delete(this.wild[i])
    ...
    public handleMessage = async (M: Message): Promise<void> => {
        ...
        const persona = getPersona(this.client.config.persona)
        ...
        if (M.chat === 'dm' && !M.message.key?.fromMe) {
            ...
            this.client.DB.getFeature('chatbot').then((feature) => {
                if (!feature.state || !text.trim()) return
                askRias(text.trim(), M.sender.jid, this.client.config.persona)
        ...
        if (M.chat === 'group' && groupData && !M.message.key?.fromMe) {
            ...
            if (groupData.autoReact) {
                const mode = (groupData.autoReactMode as ReactMode) || 'all'
                M.react(getRandomReactEmoji(mode)).catch(() => {})
            }

            // ─── Auto Cute Stickers (~12% random chance per message) ──────────
            if ((groupData as any).autoCute && Math.random() < 0.12) {
                getCuteSticker()
            ...
            // ─── Bad Words Filter ────────────────────────────────────────────────
            if ((groupData as any).badWords && (groupData as any).badWordsList?.length) {
                const msgLower = text.toLowerCase()
                const badList: string[] = (groupData as any).badWordsList || []
            ...
            const _characterMap: Record<string, { persona: string; name: string; greet: string }> = {
            ...
            const _assetMap: Record<string, { name: string; type: 'image' | 'video'; gif?: boolean }> = {
            ...
            let _matchedChar: (typeof _characterMap)[string] | null = null
            for (const hw of _helloWords) {
                if (_textLower.startsWith(hw + ' ')) {
                    const rest = _textLower.slice(hw.length + 1).trim()
                    if (_characterMap[rest]) { _matchedChar = _characterMap[rest]; break }
                }
            }
            if (_matchedChar) {
                const pa = _assetMap[_matchedChar.persona]
                const asset = this.client.assets.get(pa.name) as Buffer | undefined
                ...
                await this.client.sendMessage(M.from, this.buildMenuList(groupData?.nsfw ?? false) as any)
                return
            }
            // ───────────────────────────────────────────────────────────────────────

            // ─── Beast Mode — fires on ALL non-command group messages ─────
            const beastOn = (groupData as any).beastChat as boolean
            if (beastOn && text.trim() && !text.trim().startsWith(prefix)) {
                ;(async () => {
                    try {
                        ...
                        let beastTarget: { rawJid: string } | undefined
                        ...
                        const aiTarget = beastTarget
                            ...
                            : undefined
                        const reply = await getBeastReply(M.sender.jid, text.trim(), aiTarget)
                        await beastDelay()
            ...
            const studyAiOn   = (groupData as any).studyAi as boolean || false
            const studyAiMode = (groupData as any).studyAiMode as string || 'all'
            const studyAiShouldReply =
                studyAiOn &&
                cleanText &&
            ...
            if (studyAiShouldReply) {
                ;(async () => {
                    try {
                        await this.client.sendPresenceUpdate('composing', M.from)
                        const answer = await askRoxy(M.sender.jid, cleanText)
                        await this.client.sendPresenceUpdate('paused', M.from)
            ...
            if (isTagged && !cleanText.startsWith(prefix)) {
                ...
                if (chatbotOn && cleanText) {
                    askRias(cleanText, M.sender.jid, this.client.config.persona)
                        .then((reply) => void M.reply(reply ? persona.chatReply(reply) : persona.chatFallback()))
                        .catch(() => M.reply(persona.chatFallback()))
                }
            }
        ...
        if (text === 'gender:male' || text === 'gender:female') {
            ...
            return void await this.client.sendMessage(M.from, {
                text:
                    `🎉 *Welcome to ${this.client.config.name}!* ${emoji}\n\n` +
                    `📚 *Quick Start:*\n` +
        ...
        if (text.startsWith('studyai:')) {
            ...
            if (mode === 'off') {
                await this.client.DB.updateGroup(M.from, 'studyAi', false as any)
                return void await this.client.sendMessage(M.from, {
                    text:
                        `🔴 *Study AI — Disabled!*\n\n` +
            ...
            await this.client.DB.updateGroup(M.from, 'studyAi', true as any)
            await this.client.DB.updateGroup(M.from, 'studyAiMode', mode as any)
        ...
        if (text.startsWith('menu:')) {
            ...
            if (category === 'nsfw') {
                ...
                if (!nsfwAllowed) return void M.reply(`🔞 *NSFW is disabled in this group.*\n\nAsk an admin to enable it first.`)
            }
            const categoryIcons: Record<string, string> = {
                general: '🌐', games: '🎮', economy: '💰', fun: '🎭',
        ...
        if (text.startsWith('pvpbattle:')) {
            ...
            // Clear turn timer
            if (battle.turnTimer) { clearTimeout(battle.turnTimer); battle.turnTimer = null }
            ...
            if (action === 'defend') {
                ...
                const dmg  = calcDamage(move, attacker.pokemonLevel, defender.isDefending)
        ...
        if (!args[0] || !args[0].startsWith(prefix)) {
            ...
            if (roxyAwaitingUsers.has(M.sender.jid)) {
                if (rawTrimmed) {
                    await handleRoxyReply(
                        M.sender.jid,
                        rawTrimmed,
                        (t) => M.reply(t)
                    ).catch(() => {})
                ...
                return void this.client.log(
                    `${chalk.cyanBright('Roxy-await')} from ${chalk.yellowBright(M.sender.username)}`
                )
            }

            // 2. Prefix-free Roxy trigger: "roxy <question>" or "roxie <question>"
            const rawLower = rawTrimmed.toLowerCase()
            const roxyTrigger = ['roxy', 'roxie'].find(
                (t) => rawLower === t || rawLower.startsWith(t + ' ')
            )
            if (roxyTrigger && rawTrimmed) {
                const question = rawTrimmed.slice(roxyTrigger.length).trim()
                ;(async () => {
                    try {
                        const profile = getProfile(M.sender.jid)
                        // New user — start onboarding
                        if (profile.state === 'new') {
                            updateProfile(M.sender.jid, { state: 'awaiting_student' })
                            const lang = detectLang(question || 'hello')
                            await this.client.sendMessage(M.from, {
                                text: ROXY_INTRO(prefix, lang),
                        ...
                        await this.client.sendPresenceUpdate('composing', M.from)
                        const answer = await askRoxy(M.sender.jid, question)
                        await this.client.sendPresenceUpdate('paused', M.from)
                        if (answer) {
                            await this.client.sendMessage(M.from, {
                                text: `🌟 *Roxy*\n\n${answer}\n\n_⚡ RedzeoX × Groq_`,
                ...
                return void this.client.log(
                    `${chalk.cyanBright('Roxy-free')} from ${chalk.yellowBright(M.sender.username)} in ${chalk.blueBright(title)}`
                )
            }
        ...
        if (cmd === '') {
            ...
            const activePersona = this.client.config.persona as string
            const personaName   = getPersonaName(this.client.config.persona)
            const personaEmoji  = ({ rias: '👑', alya: '❄️', akino: '🏯', hinata: '💜', zerotwo: '🌺', miku: '🎤' } as Record<string, string>)[activePersona] ?? '🌸'
            const personaTitle  = ({ rias: '𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒕𝒐 𝑷𝒆𝒆𝒓𝒂𝒈𝒆', alya: '𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒕𝒐 𝑲𝒖𝒋𝒐𝒖 𝑾𝒐𝒓𝒍𝒅', akino: '𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒕𝒐 𝑯𝒊𝒎𝒆𝒋𝒊𝒎𝒂 𝑪𝒍𝒂𝒏', hinata: '𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒕𝒐 𝑯𝒚𝒖𝒈𝒂 𝑪𝒍𝒂𝒏', zerotwo: '𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒕𝒐 𝑺𝒒𝒖𝒂𝒅 𝟏𝟑', miku: '𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒕𝒐 𝒕𝒉𝒆 𝑪𝒐𝒏𝒄𝒆𝒓𝒕' } as Record<string, string>)[activePersona] ?? '𝑾𝒆𝒍𝒄𝒐𝒎𝒆'
            const personaQuote  = ({ rias: 'Ara ara~ Stay close~', alya: 'Apni limits mein raho~', akino: 'Fufufu~ Watching over you~', hinata: 'A-ano... Nice to meet~', zerotwo: 'For the ride, Darling~', miku: 'Enjoy the show~' } as Record<string, string>)[activePersona] ?? 'Welcome~'
            ...
            try {
                ...
                if (!introVideo) throw new Error('No intro videos found on disk')
                ...
                this.client.log(`[Intro video] failed: ${(err as Error).message}`, true)
                return void await this.client.sendMessage(M.from, {
                    text: welcomeCard,
                    footer: `⚡ RedzeoX — ${personaName}`,
                    buttons: [
        ...
        if (!command) return void M.reply(persona.commandNotFound())
        ...
        if (user.banned) return void M.reply(persona.banned())
        if (!user.tag)
        ...
        if (!(user as any).gender && !GENDER_BYPASS.includes(cmd)) {
            return void await this.client.sendMessage(M.from, {
                text:
                    `✦ ───────────── ✦\n` +
                    `    🌸 *WELCOME SETUP* 🌸\n` +
        ...
        if (command.config.category === 'dev' && !isMod)
            return void M.reply(t('mods_only', lang))
        if (M.chat === 'dm' && !command.config.dm)
            return void M.reply(t('group_only', lang))
        ...
        if (command.config.category === 'nsfw') {
            if (!groupData?.nsfw) return void M.reply(t('nsfw_only', lang))
        }
        ...
        const isGiveAmountResponse = command.name === 'give'
            && M.numbers.length >= 1
            && !M.mentioned.length
            && pendingGives.has(M.sender.jid)
            && Date.now() < (pendingGives.get(M.sender.jid)?.expiresAt ?? 0)
        ...
        if (!isGiveAmountResponse && this.cooldowns.has(`${M.sender.jid}${command.name}`)) {
            ...
            return void M.reply(t('cooldown', lang, { time: String(remainingTime) }))
        }
    ...
    private spawnRocketRaid = (): void => {
        // Runs every 3 hours — 30% chance to raid each wild-enabled group
        schedule('0 */3 * * *', async () => {
            if (this.wild.length < 1) return
            for (let i = 0; i < this.wild.length; i++) {
                if (Math.random() > 0.30) continue  // 30% chance per group per cycle
                ...
                setTimeout(async () => {
                    ...
                    try {
                        const { wild, bot } = await this.client.DB.getGroup(groupJid)
                        if (!wild) return
                        if (bot !== 'all' && bot !== this.client.config.name.split(' ')[0]) return
                        if (this.rocketRaids.has(groupJid)) return // raid already active

                        // Get group participants
                        const meta = await this.client.groupMetadata(groupJid).catch(() => null)
                        if (!meta || meta.participants.length < 2) return
                        ...
                        let victimJid = ''
                        let stolenPokemon: any = null

                        for (const p of shuffled.slice(0, 10)) {
                            if (this.client.correctJid(p.id) === this.client.correctJid(this.client.user?.id || '')) continue
                            try {
                        ...
                        if (!victimJid || !stolenPokemon) return

                        const rocket = getRocketMember()
                        ...
                        const raidAppearImg = this.client.assets.get('rocket-raid-appear') as Buffer | undefined
                        ...
                        const timer = setTimeout(async () => {
                            ...
                            const rocketWinImg = this.client.assets.get('rocket-raid-win') as Buffer | undefined
                            const failCaption =
                                `💀 *RAID FAILED!*\n\n` +
                            ...
                            ;(rocketWinImg
                                ...
                                : this.client.sendMessage(groupJid, { text: failCaption, mentions: [victimJid] })
                            ).catch(() => {})
                            ...
                            if (loseGifBuf) this.client.utils.gifToMp4(loseGifBuf).then(mp4 => this.client.sendMessage(groupJid, { video: mp4, gifPlayback: true, mimetype: 'video/mp4' })).catch(() => {})
                        }, 5 * 60 * 1000)
    ...
    public startRocketRaids = (): void => {
        this.spawnRocketRaid()
        this.client.log(`${chalk.magentaBright('Team Rocket')} raid system started 🚀`)
    }
    ...
    public loadWildEnabledGroups = async (): Promise<void> => {
        ...
        const enabledJids = new Set(allDocs.map((d: any) => d.jid))
        for (const jid of groupJids) {
            if (enabledJids.has(jid)) this.wild.push(jid)
        }
        this.client.log(
    ...
    public loadCharaEnabledGroups = async (): Promise<void> => {
        ...
        const enabledJids = new Set(allDocs.map((d: any) => d.jid))
        for (const jid of groupJids) {
            if (enabledJids.has(jid)) this.chara.push(jid)
        }
        this.client.log(
            `Successfully loaded ${chalk.blueBright(`${this.chara.length}`)} ${
                this.chara.length > 1 ? 'groups' : 'group'
            } which has enabled card spawning`
        )
        await this.spawnCard()
    }

    public loadSmashBoomGroups = async (): Promise<void> => {
        const groupJids = !this.groups ? await this.client.getAllGroups() : this.groups
        const allDocs = await this.client.DB.group.find({ jid: { $in: groupJids }, smashboom: true }).lean()
        const enabledJids = new Set(allDocs.map((d: any) => d.jid))
        for (const jid of groupJids) {
            if (enabledJids.has(jid)) this.smashboom.push(jid)
        }
        this.client.log(
            `Successfully loaded ${chalk.blueBright(`${this.smashboom.length}`)} ${
                this.smashboom.length > 1 ? 'groups' : 'group'
            } which has enabled smashboom`
        )
        this.spawnSmashBoom()
    }

    private spawnSmashBoom = (): void => {
        setInterval(async () => {
            if (this.smashboom.length < 1) return void null
            for (let i = 0; i < this.smashboom.length; i++) {
                setTimeout(async () => {
                    const jid = this.smashboom[i]
                    try {
                        const data = await this.client.DB.getGroup(jid)
                        if (!(data as any).smashboom) return void null
                        ...
                        const participants = meta.participants.filter(
                            (p) => this.client.correctJid(p.id) !== botId
                        )
                        if (!participants.length) return void null
        ...
        }, 1_000)  // every 1 second in test mode
    }

    // ─── Birthday Cron ───────────────────────────────────────────────────────
    public startBirthdayCron = (): void => {
        schedule('0 8 * * *', async () => {
            ...
            let allGroups: Record<string, { participants: { id: string }[] }> = {}
            try {
                allGroups = await this.client.groupFetchAllParticipating() as any
            } catch { /* ignore */ }

            for (const user of users) {
                const jid  = (user as any).jid as string

                // Give gold reward
                try { await this.client.DB.setCrystal(jid, 2000, 'wallet') } catch { /* ignore */ }
                ...
                for (const [groupJid, meta] of Object.entries(allGroups)) {
                    ...
                    try {
                        ...
                        if (!groupData.birthday) continue
                        await this.client.sendMessage(groupJid, {
                            text:
                                `🎂 *HAPPY BIRTHDAY!* 🎉\n\n` +
    ...
    public startLoanEmiCron = (): void => {
        schedule('0 */5 * * *', async () => {
            this.client.log(chalk.blueBright('[LOAN EMI]') + ' Running EMI collection cycle...')
            try {
                const dueUsers = await this.client.DB.getDueLoanUsers()
                if (dueUsers.length === 0) return void null

                for (const user of dueUsers) {
                    const { jid, bank, wallet, loan } = user as any
                    ...
                    if (totalAvailable >= emiAmount) {
                        ...
                        if (bankDeduct > 0) await this.client.DB.setCrystal(jid, -bankDeduct, 'bank')
                        if (walletDeduct > 0) await this.client.DB.setCrystal(jid, -walletDeduct, 'wallet')
                        ...
                        if (cleared) {
                            this.client.log(chalk.green(`[LOAN EMI] Loan CLEARED for ${phone}`))
                            try {
                                await this.client.sendMessage(jid, {
                                    text:
                                        `🎉 *LOAN CLEARED!* 🎉\n` +
                            ...
                            this.client.log(chalk.yellow(`[LOAN EMI] EMI deducted ${emiAmount} from ${phone} | Remaining: ${newRemaining}`))
                            try {
                                await this.client.sendMessage(jid, {
                                    text:
                                        `🏦 *LOAN EMI DEDUCTED*\n` +
                        ...
                        const phone = jid.split('@')[0]
                        this.client.log(chalk.red(`[LOAN EMI] Penalty applied to ${phone} | +${penalty} | New remaining: ${newRemaining}`))

                        try {
                            await this.client.sendMessage(jid, {
                                text:
                                    `⚠️ *LOAN EMI MISSED — PENALTY APPLIED!*\n` +
                                    `${'═'.repeat(28)}\n\n` +
                ...
                this.client.log(chalk.red('[LOAN EMI] Cron error: ' + err))
            }
        })
        this.client.log(chalk.blueBright('[LOAN EMI]') + ' EMI cron scheduled (every 5 hours)')
    }

    public spawnDxDGreetings = (): void => {
        const sendGreeting = async (kind: 'morning' | 'afternoon' | 'evening' | 'night') => {
            ...
            await Promise.allSettled(
                groups.map(async (group) => {
                    try {
                        ...
                        if (!(data as any).dxdGreetings) return
                        const { character, line } = getRandomGreeting(kind)
        ...
        schedule('0 7 * * *', () => void sendGreeting('morning'), tz)
        // 🌤️ Good Afternoon ~1:00 PM IST
        schedule('0 13 * * *', () => void sendGreeting('afternoon'), tz)
        // 🌇 Good Evening ~6:00 PM IST
        schedule('0 18 * * *', () => void sendGreeting('evening'), tz)
        // 🌙 Good Night ~10:00 PM IST
        schedule('0 22 * * *', () => void sendGreeting('night'), tz)
    }
    ...
    private spawnCard = async (): Promise<void> => {
        schedule('*/15 * * * *', async () => {
            ...
            if (this.cardsPaused || this.chara.length < 1) return void null
            for (let i = 0; i < this.chara.length; i++) {
                setTimeout(async () => {
                    ...
                    if (this.cardResponse.has(jid)) return void null  // unclaimed card still active

                    // ── Fetch card from Shoob.gg API; fallback to local S-tier ──────────
                    let cardTitle: string
                    let cardTier: string
                    let cardUrl: string
                    let cardSeries: string
                    let price: number

                    const apiCard = await fetchSpawnCard()
                    if (apiCard) {
                        ...
                        const fallback = getRandomCard()
                        cardTitle  = fallback.title
                        cardTier   = fallback.tier
                        cardUrl    = fallback.url
                        cardSeries = ''
                        price      = getCardPrice(fallback.tier)
                        this.client.log(`[CARD SPAWN] API failed — fallback local S-tier: ${cardTitle}`)
                    }

                    const te = (TIER_EMOJI as any)[cardTier] ?? '🃏'
                    const tn = (TIER_NAME as any)[cardTier] ?? `Tier ${cardTier}`
                    ...
                    try {
                        ...
                        if (buffer) {
                            ...
                            await this.client.sendMessage(jid, { text: caption, footer: '⚡ RedzeoX', buttonsFormat: 'buttons', buttons: cardBtns } as any)
                        }
                    } catch {
                        const cardBtns = [
                            { text: '📦 Collect Card', id: `${this.client.config.prefix}collect` },
    ...
    private moderate = async (M: Message, groupData?: TGroupModel | null): Promise<void> => {
    ...
    private handleUserStats = async (M: Message, user?: { experience: number; level: number }): Promise<void> => {
    ...
    private persistBattleDamage = async (
        jid: string,
        pokemonName: string,
        hp: number,
        maxHp: number
        ...
        try {
            ...
            const party = user.party.map((pokemon) => {
                if (pokemon.name.toLowerCase() !== target) return pokemon
                return {
                    ...pokemon,
                    hp,
                    maxHp,
    ...
    public pvpChallenges = new Map<string, { from: string; username: string; pokemon: string; expiresAt: number }>()

    public pvpBattles = new Map<string, PvpBattle>()

    public haigushaResponse = new Map<string, WaifuResponse>()

    public cardResponse = new Map<string, { card: string; cardTitle: string; tier: string; price: number }>()

    public pokemonSpawnInfo = new Map<string, {
        msgKey: import('@adiwajshing/baileys').proto.IMessageKey | null
        name: string
        level: number
        rarity: import('../lib/PokemonRarity').Rarity
        timer: ReturnType<typeof setTimeout>
    }>()

    // ── Special spawn tracking (Tier S / Event cards spawned by mods) ──────────
    public specialSpawnInfo = new Map<string, {
        msgKey: import('@adiwajshing/baileys').proto.IMessageKey | null
        title: string
        tier: string
        price: number
