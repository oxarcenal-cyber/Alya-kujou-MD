...
// ─── Badge gem color map ───────────────────────────────────────────────────────
const BADGE_GEM: Record<string, [string, string]> = {  // [center, ring]
    fire:     ['#FF6622', '#FF9944'],
    water:    ['#2288FF', '#66AAFF'],
...
function gemColor(badge: string): [string, string] {
    const k = badge.toLowerCase()
    for (const [type, colors] of Object.entries(BADGE_GEM)) {
        if (k.includes(type)) return colors
    }
...
function drawGem(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, colors: [string, string]) {
...
function drawEmptyBadge(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number) {
    ctx.save()
...
export interface TrainerCardParams {
    trainerName:   string
    trainerSprite: string   // URL of trainer sprite image
    region:        string
    regionEmoji:   string
    party:         Array<{ id: number; name: string; level: number; rarity: string }>
    gymBadges:     string[]
}

export async function buildTrainerCard(params: TrainerCardParams): Promise<Buffer> {
    ...
    try {
        const sprite = await loadImage(trainerSprite)
