...
const MAPS: Record<string, { upper: string; lower: string; digit: string }> = {
    bold: {
 ...
 */
export const convertFont = (text: string, style: string): string => {
    ...
    return [...text].map((ch) => {
        ...
        if (ui !== -1) return uArr[ui] ?? ch
        const li = LOWER.indexOf(ch)
        if (li !== -1) return lArr[li] ?? ch
        const di = DIGIT.indexOf(ch)
        if (di !== -1) return dArr[di] ?? ch
        return ch
    }).join('')
}
 ...
 */
export const previewAllFonts = (text: string): string =>
    FONT_LIST.map((f, i) =>
        `${String(i + 1).padStart(2, ' ')}. ${f.emoji} *${f.label}*\n    ${convertFont(text, f.key)}`
    ).join('\n\n')
