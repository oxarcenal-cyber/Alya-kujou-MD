...
export interface IRegion {
    ...
    starters: IStarter[]
    badgeCount: number
    image: string   // local path relative to project root
}
...
export interface ITrainerSprite {
    id: number
    name: string
    gender: '♂' | '♀'
    game: string
...
// ── Trainer Sprites ─────────────────────────────────────────────────────────
export const TRAINER_SPRITES: ITrainerSprite[] = [
...
export function getTrainerSprite(id: number): ITrainerSprite {
    return TRAINER_SPRITES.find(t => t.id === id) ?? TRAINER_SPRITES[6] // default Lucas
}

import { join } from 'path'

const REGION_IMG = (name: string) => join(__dirname, '..', '..', 'assets', 'images', 'regions', `${name}.jpg`)
...
export function getRegion(key: string): IRegion | undefined {
    return REGIONS.find(r => r.key === key.toLowerCase())
}
