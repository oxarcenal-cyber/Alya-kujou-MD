...
function pixelRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, color: string, radius = 3) {
    ctx.save()
    ctx.fillStyle = color
    ctx.beginPath()
...
function glowRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, color: string, lineW = 2, glowSize = 8) {
    ctx.save()
...
function drawStatBar(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, value: number, max: number, color: string) {
    const segments = 12
...
function drawTypeBadge(ctx: CanvasRenderingContext2D, x: number, y: number, typeName: string) {
...
// ─── OVERVIEW CARD (for !pokedex — shows all party + PC pokemon) ───────────────
export interface DexPokemon {
    name: string
    id: number
    level: number
    rarity: string
    types?: string[]    // filled later from PokeAPI
}

export async function buildDexOverviewCard(params: {
    username: string
    tag: string
    party: DexPokemon[]
    pc: DexPokemon[]
...
// ─── DETAIL CARD (for !pokedex <name>) ────────────────────────────────────────
export interface DexDetailData {
    id: number
    name: string
    level?: number
    rarity?: string
    types: string[]
    height: number    // in decimetres from PokeAPI
    weight: number    // in hectograms from PokeAPI
    stats: {
...
export async function buildDexDetailCard(data: DexDetailData): Promise<Buffer> {
    const W = 540, H = 320
    const canvas = createCanvas(W, H)
    const ctx = canvas.getContext('2d')
    ...
    // ── Stats ─────────────────────────────────────────────────────────────────
    const statEntries: [string, number, number, string][] = [
    ...
    if (data.description) {
        ...
        if (line && lineY <= H - 16) ctx.fillText(line, rx, lineY)
        ctx.restore()
    }
