...
exports.TYPE_EMOJI = void 0;
exports.getBattleHp = getBattleHp;
exports.calcDamage = calcDamage;
exports.hpBar = hpBar;
exports.fetchMovesForPokemon = fetchMovesForPokemon;
// ── Type emoji map ────────────────────────────────────────────────────────────
exports.TYPE_EMOJI = {
...
function calcDamage(move, attackerLevel, defenderIsDefending) {
...
async function fetchMovesForPokemon(pokemonName) {
    const name = pokemonName.toLowerCase();
    try {
        const pokeRes = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, { signal: AbortSignal.timeout(6000) });
        if (!pokeRes.ok)
        ...
        const candidates = pokeData.moves
            ?.slice(0, 12)
            .map((m) => m.move.name) ?? [];
        if (!candidates.length)
            return fallback;
        // Fetch all candidates in parallel
        const results = await Promise.allSettled(candidates.map((moveName) => fetch(`https://pokeapi.co/api/v2/move/${moveName}`, { signal: AbortSignal.timeout(5000) })
            .then((r) => r.json())
            .then((m) => {
            const t = m.type?.name || 'normal';
            ...
            return {
                ...
                power: m.power ?? 0,
                type: t,
                emoji: exports.TYPE_EMOJI[t] ?? '⭐',
            };
        })));
        const attackMoves = results
            .filter((r) => r.status === 'fulfilled' && r.value.power > 0)
            .map((r) => r.value)
        ...
        try {
            const r = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, { signal: AbortSignal.timeout(3000) });
            const d = await r.json();
            const t = d.types?.[0]?.type?.name || 'normal';
