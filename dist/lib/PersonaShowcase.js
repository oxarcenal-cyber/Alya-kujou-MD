...
exports.buildPersonaShowcase = buildPersonaShowcase;
...
const PUBLIC_PATH = (0, path_1.join)(__dirname, '..', '..', 'public');
// ── Helpers ───────────────────────────────────────────────────────────────────
...
// ── Main builder ──────────────────────────────────────────────────────────────
async function buildPersonaShowcase(activePersona) {
    // Layout constants
    const COLS = 3;
    const ROWS = 2;
    const CARD_W = 260;
    ...
    for (let i = 0; i < CHARACTERS.length; i++) {
        ...
        try {
            const imgBuf = await (0, fs_extra_1.readFile)((0, path_1.join)(PUBLIC_PATH, ch.file));
            const img = await (0, canvas_1.loadImage)(imgBuf);
            // Circle clip
            ctx.save();
            ctx.beginPath();
