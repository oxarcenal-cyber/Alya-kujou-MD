...
exports.buildPokemonCard = buildPokemonCard;
exports.derivePokemonCardData = derivePokemonCardData;
...
const DEFAULT_TYPE = TYPE_MAP.normal;
// ─── Helpers ──────────────────────────────────────────────────────────────────
...
function drawTypeCircle(ctx, x, y, r, color, letter) {
    // Glow
    ctx.save();
...
function drawCostDots(ctx, x, y, count, color, letter) {
    for (let i = 0; i < Math.min(count, 4); i++) {
        drawTypeCircle(ctx, x + i * 22, y, 9, color, letter);
    }
}
// ─── Main card builder ────────────────────────────────────────────────────────
async function buildPokemonCard(data) {
    ...
    // ── 3. Pokemon artwork ──────────────────────────────────────────────────
    try {
        const art = await (0, canvas_1.loadImage)(data.artUrl);
...
function pickAttackName(type, movesFromApi) {
    ...
    const apiNames = movesFromApi
        .slice(0, 20)
        .map(m => m.move.name.split('-').map((w) => w[0].toUpperCase() + w.slice(1)).join(' '))
...
function derivePokemonCardData(raw, isEx = false) {
