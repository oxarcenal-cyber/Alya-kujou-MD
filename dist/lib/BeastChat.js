...
exports.beastDelay = void 0;
exports.getBeastRoastReply = getBeastRoastReply;
exports.getBeastReply = getBeastReply;
...
let _beastClient = null;
const getBeastClient = () => {
    const key = config_1.default.GROQ_API_KEY || config_1.default.OPENAI_API_KEY;
    if (!key)
        return null;
    if (!_beastClient)
        _beastClient = new openai_1.default({
            apiKey: key,
            baseURL: config_1.default.GROQ_API_KEY ? 'https://api.groq.com/openai/v1' : undefined
        });
    return _beastClient;
};
...
const chatMemory = {
    messages: new Map(), // userId → last 20 messages
    userInfo: new Map() // userId → {name, age, location}
};
...
const pickStyle = () => BEAST_STYLES[Math.floor(Math.random() * BEAST_STYLES.length)];
...
function buildPrompt(userMessage, history, _userInfo, target) {
...
function cleanResponse(text, maxLines = 3, maxChars = 240) {
    const cleaned = text
...
async function getAIResponse(userContent, maxLines = 3, maxChars = 240) {
    ...
    try {
        const model = config_1.default.GROQ_API_KEY
        ...
        if (text)
            return cleanResponse(text, maxLines, maxChars);
    }
    catch { /* fall through */ }
    finally {
        clearTimeout(timer);
...
async function getBeastRoastReply(targetName = 'the opted-in target') {
    return getAIResponse(`[BEASTROAST MODE]
...
Roast the person's attitude or message, not protected traits. Do not make threats.`, 3, 180);
}
// ─── Public API ───────────────────────────────────────────────────────────────
...
async function getBeastReply(userId, message, target) {
    ...
    if (!reply)
        return 'bsdk teri aukat nahi thi yahan aane ki, chal nikal';
    // Save bot reply to history for context continuity
    history.push(`BEAST: ${reply}`);
    return reply;
}
/** Random human-like delay: 1–2 seconds */
const beastDelay = () => new Promise(resolve => setTimeout(resolve, Math.floor(Math.random() * 1000) + 1000));
exports.beastDelay = beastDelay;
