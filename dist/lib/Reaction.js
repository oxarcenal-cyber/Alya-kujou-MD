...
exports.Reactions = Object.fromEntries(Object.keys(REACTION_URLS).map((k) => [k, REACTION_URLS[k]]));
class Reaction {
    constructor() {
        this.getReaction = async (reaction, single = true) => {
            const url = await this.fetchUrl(REACTION_URLS[reaction] ?? `${NEKOS_BEST}pat`);
            const words = this.getSuitableWords(reaction, single);
            return { url, words };
        };
        this.fetchUrl = async (apiUrl) => {
            try {
                ...
                const fallback = await axios_1.default
                    .get(`${NEKOS_BEST}pat`, {
                    headers: { 'User-Agent': USER_AGENT },
                    timeout: 10000
                })
        ...
        this.getSuitableWords = (reaction, single = true) => {
            switch (reaction) {
