...
let default_1 = class extends Structures_1.BaseCommand {
    constructor() {
        ...
        this.execute = async (M) => {
            ...
            const aiRoast = await (0, GroqFun_1.askGroq)(`Generate ONE savage, funny, single-line roast for a person named "${targetName}". ` +
            ...
            return void M.reply(`🔥 *ROAST* 🔥\n` +
                `${'─'.repeat(25)}\n\n` +
                `@${targetName} — ${roast}\n\n` +
                `${'─'.repeat(25)}\n` +
                `📢 *How to use:* \`${prefix}roast @user\`\n` +
                `_Compliment ke liye: \`${prefix}compliment @user\`_`, 'text', undefined, undefined, undefined, [target]);
        };
    }
};
default_1 = __decorate([
    (0, Structures_1.Command)('roast', {
