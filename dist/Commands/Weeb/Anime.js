    ...
    constructor() {
        ...
        this.execute = async (M, { context }) => {
            ...
            await this.client.utils
                .withRetry(() => new marika_1.Anime().searchAnime(query))
                .then(async ({ data }) => {
                const result = data[0];
                if (!result)
                    return void M.reply((0, lib_1.t)('weeb_anime_not_found', lang, { query }));
                ...
                .catch((error) => {
                console.error(`[Anime] Failed to fetch anime for "${query}":`, error?.message);
                return void M.reply((0, lib_1.t)('weeb_anime_not_found', lang, { query }));
            });
        };
    }
