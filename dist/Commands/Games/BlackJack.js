...
const BlackjackRenderer_1 = require("../../lib/BlackjackRenderer");
// ─── State ────────────────────────────────────────────────────────────────────
const games = new Map();
...
function newGameListBtn(p) {
    return [{ text: '🃏 New Game', sections: [{ title: '💰 Choose Bet Amount', rows: newGameRows(p) }] }];
}
// ─── Image sender ─────────────────────────────────────────────────────────────
async function sendTable(client, from, game, status, hideDealer, resultMsg, quoted, dealerExtra // extra info line for dealer phase
) {
    const pv = handValue(game.playerHand);
    const dv = handValue(game.dealerHand);
    const p = client.config.prefix;
    const buf = await (0, BlackjackRenderer_1.renderBlackjackTable)({
...
// ─── Lose GIF sender ──────────────────────────────────────────────────────────
function sendLoseGif(client, from, bet, prefix) {
    const key = LOSE_KEYS[Math.floor(Math.random() * LOSE_KEYS.length)];
    const gifBuf = client.assets.get(key);
    if (!gifBuf)
        return;
    client.utils.gifToMp4(gifBuf)
        .then((mp4) => client.sendMessage(from, {
        video: mp4,
        gifPlayback: true,
        mimetype: 'video/mp4',
    ...
    constructor() {
        ...
        this.execute = async (M, { context, args }) => {
            ...
            if (input === 'motivate') {
                ...
                return void await this.client.sendMessage(M.from, {
                    text: `🌟 *Daily Motivation — Just for You*\n\n` +
                        `${quote}\n\n` +
                        `━━━━━━━━━━━━━━━━━━\n` +
            ...
            if (input === 'hit') {
                ...
                if (game.phase === 'done')
                    return void M.reply(`❌ Game over! Start new: \`${prefix}bj <amount>\``);
            ...
            if (input === 'stand') {
                ...
                if (game.phase === 'done')
                    return void M.reply('❌ Game already over!');
                return void this.resolveGame(M, game, key);
            }
            // ── Already in game ───────────────────────────────────────────────
            ...
            if (handValue(pHand) === 21)
                return void this.resolveGame(M, newGame, key);
            return void await sendTable(this.client, M.from, newGame, 'playing', true, undefined, M.message);
        };
        // ── Resolve: dealer plays ──────────────────────────────────────────────
        this.resolveGame = async (M, game, key) => {
            const prefix = this.client.config.prefix;
            game.phase = 'done';
            // Reveal dealer's hand
            await sendTable(this.client, M.from, game, 'dealer', false, undefined, M.message, `🎴 Dealer reveals hidden card!`);
            await sleep(1200);
            ...
            while (handValue(game.dealerHand) < 17) {
                ...
                const dv = handValue(game.dealerHand);
                await sleep(800);
                await sendTable(this.client, M.from, game, 'dealer', false, undefined, M.message, dv >= 17
...
default_1 = __decorate([
    (0, Structures_1.Command)('blackjack', {
