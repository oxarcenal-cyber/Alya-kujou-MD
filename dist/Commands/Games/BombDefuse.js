...
function buildBombFrame(timeLeft, wires, cut) {
    const bar = timeLeft > 0
        ? '█'.repeat(timeLeft) + '░'.repeat(10 - timeLeft)
        : '░'.repeat(10);
    const wireList = wires.map((w, i) => `  ${cut && WIRE_KEYS[i] === cut ? '✂️' : '🔌'} ${w}`).join('\n');
    return (`💣 ═══ *BOMB DEFUSE* ═══ 💣\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
        ...
        this.execute = async (M, { context }) => {
            ...
            if (input.startsWith('cut')) {
                ...
                if (chosenKey === game.safeWire) {
                    ...
                    return void await this.client.sendMessage(M.from, {
                        text: `✂️ *${WIRE_COLORS[idx]} Wire Kata!*\n\n` +
                            `💣 BEEP...\n` +
                            `💣 BEEP BEEP...\n` +
                            `💥 *BOOM!!!*\n\n` +
                            `💀 *TERA KAAM TAMAM!*\n\n` +
            ...
            // ── Already in game ───────────────────────────────────────────────
            if (game)
                return void M.reply(`❌ Bomb pehle se active hai!\n` +
