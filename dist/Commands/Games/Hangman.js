    ...
    constructor() {
        ...
        this.execute = async (M, { context }) => {
            ...
            if (input === 'start' || input === 'new') {
                ...
                return void M.reply(`🎯 *HANGMAN SHURU!*\n\n` +
                    `\`\`\`${stages[0]}\`\`\`\n\n` +
            ...
            if (game.guessed.includes(input))
                return void M.reply(`⚠️ *"${input.toUpperCase()}"* pehle se try kiya!\n🔡 *Used:* ${game.guessed.sort().join(', ')}`);
            ...
            return void M.reply(`${correct ? `✅ *"${input.toUpperCase()}"* sahi hai!` : `❌ *"${input.toUpperCase()}"* nahi hai!`}\n\n` +
                `\`\`\`${stages[game.wrong]}\`\`\`\n\n` +
                `🔤 *Word:* ${display}\n` +
                `💀 *Wrong:* ${game.wrong}/6\n` +
                `🔡 *Used:* ${game.guessed.sort().join(', ')}\n\n` +
        ...
        this.getDisplay = (word, guessed) => word.split('').map(l => (guessed.includes(l) ? l.toUpperCase() : '_')).join(' ');
    }
};
default_1 = __decorate([
    (0, Structures_1.Command)('hangman', {
