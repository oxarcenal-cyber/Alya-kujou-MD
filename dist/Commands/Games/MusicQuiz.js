...
let default_1 = class extends Structures_1.BaseCommand {
    constructor() {
        ...
        this.execute = async (M, { context }) => {
            ...
            const sendWithNextBtn = async (text) => {
            ...
            if (input === 'stop' || input === 'end') {
                if (!quiz)
                    return void M.reply('❌ Koi quiz nahi chal raha!');
                activeQuizzes.delete(group);
                return void sendWithNextBtn(`🛑 *Quiz Stopped!*\n\n` +
            ...
            if (input === 'skip') {
                ...
                activeQuizzes.delete(group);
                return void sendWithNextBtn(`⏭️ *Skipped!*\n\n` +
            ...
            if (input === 'hint') {
                if (!quiz)
                    return void M.reply(`❌ Koi quiz nahi!\n📢 Shuru karo: \`${prefix}mq start\``);
                if (quiz.hints >= 2)
                    return void M.reply('❌ Max 2 hints already use ho gaye!');
                quiz.hints++;
                // FIX: was inferred as never[] causing TS2345 — now explicitly typed as string[]
                const hints = [];
                if (quiz.hints >= 1)
                    hints.push(`🎤 Artist: *${quiz.song.artist}*`);
                if (quiz.hints >= 2)
                    hints.push(`🎸 Genre: *${quiz.song.genre}*`);
                if (quiz.hints >= 2)
                    hints.push(`📝 First letter: *${quiz.song.title[0].toUpperCase()}*`);
                return void await this.client.sendMessage(M.from, {
                    text: `💡 *HINT ${quiz.hints}/2:*\n\n` +
                        hints.join('\n') +
            ...
            if (input === 'start' || input === 'new') {
                if (quiz)
                    return void M.reply(`❌ Quiz pehle se chal raha hai!\n` +
            ...
            if (Date.now() > quiz.expiresAt) {
                activeQuizzes.delete(group);
                return void sendWithNextBtn(`⏰ *Time Up!*\n\n` +
                    `🎵 Answer tha: *${quiz.song.title}*\n` +
                    `🎤 Artist: *${quiz.song.artist}*`);
            }
            ...
            if (correct) {
                ...
                return void sendWithNextBtn(`🎉 *CORRECT!* 🎉\n\n` +
...
default_1 = __decorate([
    (0, Structures_1.Command)('musicquiz', {
