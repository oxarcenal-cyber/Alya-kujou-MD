...
function buildCylinder(chamber, total = 6) {
...
let default_1 = class extends Structures_1.BaseCommand {
    constructor() {
        ...
        this.execute = async (M, { context }) => {
            ...
            // ── Help ──────────────────────────────────────────────────────────
            if (!input)
                return void M.reply(`🔫 *RUSSIAN ROULETTE*\n\n` +
                    `📢 *How to use:*\n` +
            ...
            if (input === 'join') {
                const g = rrGames.get(group);
                if (!g)
                    return void M.reply(`❌ Koi game nahi!\n📢 Banao: \`${prefix}rr create <amount>\``);
                if (g.started)
                    return void M.reply('❌ Game already shuru ho gayi!');
                if (g.players.length >= 6)
                    return void M.reply('❌ Max 6 players!');
                if (g.players.find(p => p.jid === fromJid))
                    return void M.reply('❌ Pehle se join ho!');
                const { wallet } = await this.client.DB.getUser(fromJid);
                if (wallet < g.bet)
                    return void M.reply(`❌ Tumhare paas *${g.bet.toLocaleString()} gold* nahi!`);
                g.players.push({ jid: fromJid, name: M.sender.username || 'Player', alive: true });
                return void M.reply(`✅ *${M.sender.username || 'Player'}* joined!\n\n` +
            ...
            if (input === 'cancel') {
                ...
                if (!g)
                    return void M.reply('❌ Koi game nahi!');
                if (g.hostJid !== fromJid && !M.sender.isAdmin)
            ...
            if (input === 'start') {
                ...
                for (const p of g.players)
                    await this.client.DB.setCrystal(p.jid, -g.bet);
            ...
            if (input === 'shoot') {
                const g = rrGames.get(group);
                if (!g || !g.started)
                    return void M.reply(`❌ Koi active game nahi!\n📢 Banao: \`${prefix}rr create <amount>\``);
                ...
                if (!currentPlayer)
                    return void M.reply('❌ Game error! Try again.');
                ...
                g.chamber++;
                const sent = await this.client.sendMessage(M.from, { text: `🔫 *${currentPlayer.name}* ne trigger dabaya...\n\n${buildCylinder(chamberNum)}\n\n_Click!_` }, { quoted: M.message });
                if (!sent?.key)
                    return;
                ...
                if (bullet === 1) {
                    ...
                    if (loseGifBuf)
                        this.client.utils.gifToMp4(loseGifBuf).then(mp4 => this.client.sendMessage(M.from, { video: mp4, gifPlayback: true, mimetype: 'video/mp4', caption: `💀 *${currentPlayer.name} ELIMINATED!*` })).catch(() => { });
                    ...
                    return void await this.client.sendMessage(M.from, {
                        text: `💀 *${currentPlayer.name}* eliminated!\n\n` +
                    ...
                    await sleep(800);
                    return void await this.client.sendMessage(M.from, {
                        text: `😅 *${currentPlayer.name}* bach gaya!\n\n` +
...
default_1 = __decorate([
    (0, Structures_1.Command)('russianroulette', {
