...
// ─── Render ───────────────────────────────────────────────────────────────────
function renderTrack(players, round, finished = false) {
    let text = `🏁 ═══ *GROUP RACE* ═══ 🏁\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
    ...
    constructor() {
        ...
        this.execute = async (M, { context }) => {
            ...
            if (input === 'join') {
                ...
                return void M.reply(`✅ *${M.sender.username || 'Racer'}* joined! ${emoji}\n\n` +
            ...
            if (input === 'start') {
                ...
                await M.reply(`🏎️ *RACE SHURU HO RAHI HAI!*\n\n` +
                    race.players.map(p => `${p.emoji} *${p.name}*`).join('\n') +
                    `\n\n💰 Prize Pool: *${(race.bet * race.players.length).toLocaleString()} gold*\n\n` +
                    `_3... 2... 1... GO! 🏁_`);
                await sleep(2000);
                const sent = await this.client.sendMessage(M.from, { text: renderTrack(race.players, 1) }, { quoted: M.message });
                if (!sent?.key) {
                    races.delete(group);
                    return;
                }
