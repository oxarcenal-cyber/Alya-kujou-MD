...
const activeCDs = new Set();
const sleep = (ms) => new Promise(r => setTimeout(r, ms));
// ─── Helpers ─────────────────────────────────────────────────────────────────
function dealHand(count = 4) {
...
function renderFrame(f1, f2, round, log) {
    return (`🃏 ═══ *CARD DUEL* ═══ 🃏\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `⚡ Round ${round}\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
...
function applyAttack(attacker, defender, log) {
    ...
    constructor() {
        ...
        this.execute = async (M, { args, context }) => {
            ...
            if (input === 'accept') {
                ...
                if (!session)
                    return void M.reply(`❌ Koi pending card duel nahi!\n📢 Challenge: \`${prefix}cd @user 1000\``);
                ...
                if (cData.wallet < session.bet)
                    return void M.reply(`❌ Challenger ke paas gold nahi!`);
                if (dData.wallet < session.bet)
                    return void M.reply(`❌ Tumhare paas *${session.bet.toLocaleString()} gold* nahi!`);
                pending.delete(group);
                ...
                // Show card draw
                await M.reply(`🃏 *CARD DUEL ACCEPTED!*\n\n` +
                ...
                const sent = await this.client.sendMessage(M.from, { text: renderFrame(f1, f2, 1, log) }, { quoted: M.message });
                if (!sent?.key) {
                    activeCDs.delete(f1.jid);
                    activeCDs.delete(f2.jid);
                    return;
                }
                const edit = (t) => this.client.sendMessage(M.from, { text: t, edit: sent.key });
                let round = 1;
                while (f1.currentHp > 0 && f2.currentHp > 0 && round <= 30) {
                    ...
                    await sleep(1000);
                    await edit(renderFrame(f1, f2, round, log));
                ...
                const loseGifBuf = this.client.assets.get(loseGifKey);
                if (loseGifBuf)
                    this.client.utils.gifToMp4(loseGifBuf).then(mp4 => this.client.sendMessage(M.from, { video: mp4, gifPlayback: true, mimetype: 'video/mp4', caption: `💔 *${loser.name} haar gaya!*` })).catch(() => { });
                return;
            }
            // ── Cancel ────────────────────────────────────────────────────────
            if (input === 'cancel') {
                const session = pending.get(group);
                if (!session)
                    return void M.reply('❌ Koi pending duel nahi!');
            ...
            if (targetJid === selfJid)
                return void M.reply('❌ Khud se duel nahi!');
            const botJid = this.client.correctJid(this.client.user?.id || '');
            if (targetJid === botJid)
                return void M.reply('❌ Bot se nahi!');
            if (activeCDs.has(selfJid))
                return void M.reply('❌ Tum pehle se ek duel mein ho!');
            if (activeCDs.has(targetJid))
...
default_1 = __decorate([
    (0, Structures_1.Command)('cardduel', {
