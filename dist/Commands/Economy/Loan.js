    ...
    constructor() {
        ...
        this.execute = async (M) => {
            ...
            if (M.numbers.length < 1) {
                return void M.reply(`🏦 *LOAN SYSTEM*\n` +
                    `${'─'.repeat(30)}\n\n` +
                    `💡 *How it works:*\n` +
            ...
            if (amount < MIN_LOAN)
                return void M.reply(`❌ *Minimum loan amount is ${MIN_LOAN.toLocaleString()} 💰*\n\n` +
                    `📢 *Usage:* \`${prefix}loan <amount>\``);
            if (amount > MAX_LOAN)
                return void M.reply(`❌ *Maximum loan amount is ${MAX_LOAN.toLocaleString()} 💰*\n\n` +
                    `📢 *Usage:* \`${prefix}loan <amount>\``);
            const user = await this.client.DB.getUser(M.sender.jid);
            if (user.loan?.active)
                return void M.reply(`❌ *You already have an active loan!*\n\n` +
            ...
            return void M.reply(`✅ *LOAN APPROVED!* 🏦\n` +
                `${'═'.repeat(30)}\n\n` +
