...
function buildGiveawayFrame(session, timeLeftMs) {
...
let default_1 = class extends Structures_1.BaseCommand {
    constructor() {
        ...
        this.execute = async (M, { context }) => {
            ...
            // ── Help ──────────────────────────────────────────────────────────
            if (!input)
                return void M.reply(`🎟️ *GIVEAWAY SYSTEM*\n\n` +
                    `📢 *How to use:*\n` +
            ...
            if (input === 'join') {
                ...
                if (!session)
                    return void M.reply(`❌ Koi active giveaway nahi!\n📢 Create: \`${prefix}ga create <amount> <time>\``);
                ...
                return void M.reply(`✅ *${M.sender.username || 'User'}* giveaway mein join hua! 🎟️\n\n` +
                    `💰 Prize: *${session.prize.toLocaleString()} gold*\n` +
            ...
            if (input.startsWith('create')) {
                ...
                const sent = await this.client.sendMessage(M.from, { text: buildGiveawayFrame(session, timeMs) }, { quoted: M.message });
                ...
                const updateInterval = setInterval(async () => {
                    ...
                    await this.client.sendMessage(group, { text: buildGiveawayFrame(s, left), edit: s.msgKey });
                }, 15000);
        ...
        this.pickWinner = async (M, session) => {
            if (session.participants.length === 0) {
                return void M.reply(`🎟️ *GIVEAWAY ENDED*\n\n` +
                    `😢 Koi join hi nahi kiya!\n` +
                    `💰 *${session.prize.toLocaleString()} gold* wapas host ko.`);
            }
            // Animate winner selection
            const sent = await this.client.sendMessage(M.from, { text: `🎰 *Winner choose ho raha hai...*\n\n${session.participants.map(p => p.name).join(', ')}\n\n_Spinning..._` }, { quoted: M.message });
            await sleep(1500);
...
default_1 = __decorate([
    (0, Structures_1.Command)('giveaway', {
