 ...
 */
function frame(pos, bet, amt, phase, result) {
    ...
    const spinLine = phase === 'fast' ? '🌀 *Spinning fast...* ⚡' :
        phase === 'slow' ? '🐌 *Slowing down...* 💫' :
            '';
    let text = `🎡 ═══ *ROULETTE ROYALE* ═══ 🎡\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
        `${colorRow}\n` +
        `${numRow}\n` +
        `         ↑\n` +
...
function parseBet(input) {
    ...
    if (s === 'red')
        return { label: '🔴 Red', check: n => n > 0 && RED_NUMS.has(n), multiplier: 1 };
    if (s === 'black')
        return { label: '⚫ Black', check: n => n > 0 && !RED_NUMS.has(n), multiplier: 1 };
    if (s === 'green')
        return { label: '🟢 Green (0)', check: n => n === 0, multiplier: 17 };
    if (s === 'odd')
        return { label: '🔢 Odd', check: n => n > 0 && n % 2 !== 0, multiplier: 1 };
    if (s === 'even')
        return { label: '🔢 Even', check: n => n > 0 && n % 2 === 0, multiplier: 1 };
    if (s === 'low')
        return { label: '📉 Low (1-18)', check: n => n >= 1 && n <= 18, multiplier: 1 };
    if (s === 'high')
        return { label: '📈 High (19-36)', check: n => n >= 19 && n <= 36, multiplier: 1 };
...
let default_1 = class extends Structures_1.BaseCommand {
    constructor() {
        ...
        this.execute = async (M, { args }) => {
            ...
            // ── Input validation ──────────────────────────────────────────────
            if (args.length < 2)
                return void M.reply(`🎡 *ROULETTE ROYALE*\n\n` +
            ...
            if (!bet)
                return void M.reply(`❌ Sahi bet type likho!\n` +
            ...
            const sent = await this.client.sendMessage(M.from, { text: frame(initPos, bet.label, amount, 'fast') }, { quoted: M.message });
            if (!sent?.key)
                return;
            // ── Play animation ────────────────────────────────────────────────
            ...
            if (!win) {
                ...
                if (gifBuf)
                    this.client.utils.gifToMp4(gifBuf).then(mp4 => this.client.sendMessage(M.from, { video: mp4, gifPlayback: true, mimetype: 'video/mp4', caption: `💔 *Haar gaye! -${amount.toLocaleString()} gold!*` })).catch(() => { });
            }
        };
    }
};
default_1 = __decorate([
    (0, Structures_1.Command)('roulette', {
