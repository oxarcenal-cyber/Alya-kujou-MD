...
const frameHeader = (round) => `⚔️ ═══ *DUEL BATTLE* ═══ ⚔️\n` +
    `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
    `             ⚡ Round ${round}\n` +
    `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
function buildFrame(a, b, round, log) {
    const lines = log.slice(-4); // show last 4 log lines max
    return (frameHeader(round) +
        `${a.emoji} *${a.name}*\n${hpBar(a.hp, a.maxHp)}\n\n` +
        `${b.emoji} *${b.name}*\n${hpBar(b.hp, b.maxHp)}\n\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        lines.map(l => `  ${l}`).join('\n'));
...
// ─── Battle Engine ────────────────────────────────────────────────────────────
async function runBattle(M, client, attacker, defender, bet) {
    const log = ['⚡ Battle Start!', ''];
    let round = 1;
    // Initial frame
    const sent = await client.sendMessage(M.from, { text: buildFrame(attacker, defender, round, log) }, { quoted: M.message });
    if (!sent?.key)
        return;
    const edit = async (text) => {
        await client.sendMessage(M.from, { text, edit: sent.key });
    };
    await sleep(1500);
    // ── Battle loop ──────────────────────────────────────────────────────────
    ...
    await sleep(1200);
    const finalText = `⚔️ ═══ *DUEL BATTLE* ═══ ⚔️\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
    ...
    if (loseGifBuf)
        client.utils.gifToMp4(loseGifBuf).then(mp4 => client.sendMessage(M.from, { video: mp4, gifPlayback: true, mimetype: 'video/mp4', caption: `💔 *${loser.name} haar gaya!*` })).catch(() => { });
    ...
    constructor() {
        ...
        this.execute = async (M, { args, context }) => {
            ...
            if (M.mentioned.length < 1)
                return void M.reply(`⚔️ *DUEL SYSTEM*\n\n` +
                    `📢 *Challenge karo:*\n` +
            ...
            if (pendingDuels.has(group))
                return void M.reply(`❌ Is group mein pehle se ek duel pending hai!\n` +
