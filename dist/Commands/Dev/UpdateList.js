    ...
    constructor() {
        ...
        this.execute = async (M, args) => {
            ...
            if (sub === 'resolve' || sub === 'reject') {
                ...
                if (!id)
                    return void await M.reply(`❌ Please provide an ID!\n\n*Usage:*\n\`${this.client.config.prefix}updatelist resolve <id>\``);
                const newStatus = sub === 'resolve' ? 'done' : 'rejected';
                const updated = await this.client.DB.updateFeedbackStatus(id, newStatus, note);
                if (!updated)
                    return void await M.reply(`❌ ID \`${id}\` not found or already closed.`);
                const emoji = newStatus === 'done' ? '✅' : '❌';
                return void await M.reply(`${emoji} *Feedback ${newStatus.toUpperCase()}!*\n\n` +
                    `🆔 ID: \`${id}\`\n` +
                    (note ? `📝 Note: _"${note}"_\n` : '') +
            ...
            const PAGE_SIZE = 5;
            const items = await this.client.DB.getFeedbackList(filter, pageArg, PAGE_SIZE);
            const total = await this.client.DB.getFeedbackCount(filter);
            const pages = Math.ceil(total / PAGE_SIZE) || 1;
            if (items.length === 0) {
                return void await M.reply(`📭 *No ${sub === 'all' ? '' : sub + ' '}feedback found!*\n\n` +
...
default_1 = __decorate([
    (0, Structures_1.Command)('updatelist', {
