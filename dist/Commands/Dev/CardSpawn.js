...
// ── Card pools ─────────────────────────────────────────────────────────────
const S_CARDS = CardData_1.ALL_CARDS.filter(c => c.tier === 'S');
const EVT_CARDS = S_CARDS.filter(c => (0, CardData_1.isGif)(c.url)); // Event = animated S cards
function pick(arr) { return arr[Math.floor(Math.random() * arr.length)]; }
// ── Caption builder ─────────────────────────────────────────────────────────
function buildCaption(isEvent, title, price, spawnedBy, prefix, status, series) {
    const seriesLine = series ? `  ‧₊˚ 🎬 𝑺𝒆𝒓𝒊𝒆𝒔  ·❀·  ${series}\n` : '';
    if (isEvent)
        return (`˚✧. ୭ৎ 𝒍𝒍.𝑶'𝑶𝒅𝒚𝒔𝒔𝒆𝒚 ୭ৎ .✧˚\n\n` +
            `  🌸✿ᰰ  *${title}*  ✿ᰰ🌸\n` +
...
// ── Command ─────────────────────────────────────────────────────────────────
let CardSpawnCommand = class CardSpawnCommand extends Structures_1.BaseCommand {
    constructor() {
        super(...arguments);
        this.execute = async (M, { context }) => {
            ...
            // ── Usage hint ─────────────────────────────────────────────────────
            if (!isEvent && !isS)
                return void M.reply(`📖 *CardSpawn — Mod Only Command*\n\n` +
                    `*Usage:*\n` +
            ...
            if (this.handler.cardResponse.has(M.from))
                return void M.reply(`⚠️ A card is already active in this group!\n` +
                    `Wait for someone to \`${prefix}collect\` it or wait 15 minutes.`);
            // ── Pick card + price ───────────────────────────────────────────────
            const card = pick(pool);
            const price = isEvent
                ? Math.floor(Math.random() * (800000 - 200000) + 200000)
            ...
            const gif = (0, CardData_1.isGif)(card.url);
            try {
                let buf = null;
                if (gif) {
                ...
                if (buf) {
                    ...
                    const sent = await this.client.sendMessage(M.from, { text: caption, footer: '⚡ RedzeoX', buttonsFormat: 'buttons', buttons: spawnBtns });
                    sentKey = sent?.key ?? null;
                }
            }
            catch {
...
CardSpawnCommand = __decorate([
    (0, Structures_1.Command)('cardspawn', {
...
exports.default = CardSpawnCommand;
