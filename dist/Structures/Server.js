...
const CommandTester_1 = require("../test/CommandTester");
class Server {
    constructor(client) {
        this.client = client;
        this.path = (0, path_1.join)(__dirname, '..', '..', 'public');
        this.app = (() => {
        ...
        this.logBuffer = [];
        this.sseClients = [];
        this.app.use('/assets', express_1.default.static(this.path));
        ...
        this.app.post('/pair', express_1.default.json(), async (req, res) => {
            const { phone } = req.body || {};
            if (!phone)
                return void res.status(400).json({ error: 'Phone number required' });
            try {
                ...
                const codePromise = new Promise((resolve, reject) => {
                    ...
                    setTimeout(() => reject(new Error('Pairing code timeout — try again')), 30000);
                });
                // Force reconnect: close current socket so it restarts and fires a fresh QR event
                try {
                    client.end?.(new Error('pairing-restart'));
                }
                catch { }
    ...
    async function downloadZip() {
      ...
      try {
        ...
        this.app.get('/download', (_req, res) => {
            ...
            const out = (0, path_1.join)(root, 'bot-backup.zip');
        ...
        this.app.post('/api/run-tests', (_req, res) => {
            try {
                const tester = new CommandTester_1.CommandTester();
  ...
  <script>
    ...
    function escHtml(str) {
      return String(str || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
        ...
        const INTRO_VIDEO_DIR = (0, path_1.join)(__dirname, '..', '..', 'assets', 'videos');
        const checkVideoKey = (req, res) => {
        ...
        this.app.get('/api/videos', (req, res) => {
            if (!checkVideoKey(req, res))
                return;
            try {
                const files = (0, fs_1.readdirSync)(INTRO_VIDEO_DIR)
        ...
        this.app.get('/api/video/random', (req, res) => {
            if (!checkVideoKey(req, res))
                return;
            try {
                const files = (0, fs_1.readdirSync)(INTRO_VIDEO_DIR).filter(f => /^intro-\d+\.mp4$/.test(f));
                if (!files.length)
                    return void res.status(404).json({ error: 'No intro videos found' });
                const picked = files[Math.floor(Math.random() * files.length)];
                const videoPath = (0, path_1.join)(INTRO_VIDEO_DIR, picked);
        ...
        this.app.get('/api/video/:name', (req, res) => {
            ...
            if (!/^intro-\d+$/.test(name))
                return void res.status(400).json({ error: 'Invalid video name. Use format: intro-1' });
            const videoPath = (0, path_1.join)(INTRO_VIDEO_DIR, `${name}.mp4`);
            if (!(0, fs_1.existsSync)(videoPath))
                return void res.status(404).json({ error: `Video "${name}" not found` });
            res.setHeader('Content-Type', 'video/mp4');
        ...
        this.client.log = (text, error = false) => {
    ...
    captureLog(text, error = false) {
        const clean = text.replace(/\x1B\[[0-9;]*m/g, '').trim();
        if (!clean || this.shouldFilter(clean))
            return;
        const entry = {
